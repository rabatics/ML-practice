...
public class mlclass {
		...
		double theta1=0;
			...
			do{
			 ...
			 for(int i=1;i<instances.numInstances();i++){
				...
				sumnt0=sumnt0+(( theta0+(theta1*is.value(0)) )-is.value(1));
	...
	public static double costFunction(Instances i,double t0,double t1){
		 double cost=0;
		 double interm=0;
		for(int i1=1; i1<i.numInstances();i1++){
			Instance is=i.instance(i1);
			System.out.println(is.value(0)+" ---- "+is.value(1));
			double er=(t0+t1*is.value(0) )-is.value(1);
